...
cdef class Rounder:
    ...
    cpdef double size(self, double size):
        ...
        """
        cdef double value = self.lot_sz * floor(size * self._inverse_lot_sz)
    ...
    cpdef cnp.ndarray bids(self, cnp.ndarray pxs):
        ...
        cdef:
            ...
            # Local copies of instance vars for faster access in the loop
    ...
    cpdef cnp.ndarray sizes(self, cnp.ndarray sizes):
        ...
        cdef:
            ...
            Py_ssize_t n = sizes_view.shape[0]
            cnp.ndarray result = np.empty(n, dtype=np.double)
            double[:] result_view = result
            Py_ssize_t i
        ...
        for i in range(n):
            result_view[i] = self._round_to(
                lot_sz * floor(sizes_view[i] * inverse_lot_sz),
                lot_rounding_factor
            )
        return result
