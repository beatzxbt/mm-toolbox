...
cdef class ArrayOrderbook:
    def __init__(
        ...
        if size <= 2:
            raise ValueError(f"Invalid size; expected >3 but got {size}")

        self._tick_size = tick_size
        self._lot_size = lot_size
        self._size = size

        # Kept as Python objects here for faster returns.
        self._asks = np.zeros((self._size, 2), dtype=np.double)
        self._bids = np.zeros((self._size, 2), dtype=np.double)

        # Maintain memoryview of above ndarray objects for C-like 
        # indexing performance. Native C arrays can be used for 20-35% 
        # speed improvement, though out-of-bounds checks and index 
        # loopback being disabled bring the speed to near par.
        self._asks_view = self._asks
        self._bids_view = self._bids
    ...
    cdef inline void _reset(self):
        ...
        # Iterating over the view's and setting each index
        # to 0 may be faster, but for now this works.
        cnp.PyArray_FillWithScalar(self._asks, 0.0)
        cnp.PyArray_FillWithScalar(self._bids, 0.0)
    ...
    cdef inline void _roll_bids(
        ...
        Example for shift_right = True:
            ...
            Rolling the array right from i=2 produces:
                [[65, 1], [64, 1], [62, 2], [62, 2]] 
            then we overwrite row=2 with [63, 1]:
                [[65, 1], [64, 1], [63, 1], [62, 2]]

        Example for shift_right = False:
            If the best bid at i=0 is removed, we shift everything left from start_idx=0:
    ...
    cdef inline void _roll_asks(
        ...
        Example (shift_right = True):
            ...
            and we want to insert [102, 1] at i=2.

            Rolling the array right from i=2 results in:
            [[100, 1], [101, 2], [103, 1], [103, 1]]
            then we overwrite row=2 with [102, 1]:
            [[100, 1], [101, 2], [102, 1], [103, 1]]

        Example (shift_right = False):
            If removing the best ask at i=0, we shift everything left:
            old: [[100, 1], [101, 2], [103, 1], [105, 1]]
            new: [[101, 2], [103, 1], [105, 1], [dummy_px, 0]]
            (You can fill the final row with a dummy if needed.)
        """
        cdef: 
            Py_ssize_t i
            Py_ssize_t end = self._size
    ...
    cpdef void warmup(self, cnp.ndarray new_asks, cnp.ndarray new_bids, Py_ssize_t new_seq_id):
        ...
        """
        cdef:
            double[:, :] new_asks_view = new_asks 
            double[:, :] new_bids_view = new_bids 
            Py_ssize_t bid_dims = new_bids_view.ndim
            Py_ssize_t ask_dims = new_asks_view.ndim
            Py_ssize_t bid_len = new_bids_view.shape[0]
            Py_ssize_t ask_len = new_asks_view.shape[0]
        
        if ask_dims != 2 or bid_dims != 2:
            raise IndexError(f"Invalid dimensions; expected 2D but got bid_dims({bid_dims}D) & ask_dims({ask_dims}D)")

        if ask_len != self._size or bid_len != self._size:
            raise IndexError(f"Invalid length; expected {self._size} but got bid_len({bid_len}) & ask_len({ask_len})")
    ...
    cpdef void update_bbo(
        ...
        cdef:
            ...
            # this will count up to 2. If it does, we can return.
    ...
    cpdef void update_asks(
        self,
        cnp.ndarray updated_asks,
        Py_ssize_t new_seq_id
    ):
        """
        ...
        """
        cdef:
            double[:, :] new_asks_view = updated_asks
            ...
            double new_ask_px, new_ask_sz
            Py_ssize_t i

        self.ensure_warm()
        
        if new_asks_dims != 2:
            raise IndexError(f"Invalid dimensions; expected 2D but got {new_asks_dims}D")

        if new_asks_len < 1:
            raise IndexError(f"Invalid length; expected >=1 but got {new_asks_len}")
        ...
        # We assume updated_asks is sorted ascending by price.
    ...
    cpdef void update_bids(
        ...
        cdef:
            double[:, :] new_bids_view = updated_bids
            ...
            double new_bid_px, new_bid_sz
            Py_ssize_t i

        if new_bids_dims != 2:
            raise IndexError(f"Invalid dimensions; expected 2D but got {new_bids_dims}D")

        if new_bids_len < 1:
            raise IndexError(f"Invalid length; expected >=1 but got {new_bids_len}")
        ...
        # We assume updated_bids is sorted ascending by price. 
    ...
    cpdef void update_full(
        ...
        Raises:
            RuntimeError: If the provided arrays are not 2D or have no rows.
        """
        self.ensure_warm()
    ...
    cpdef double get_slippage(self, double size, bint is_bid, bint is_base_currency=False):
        """
        Calculate the average slippage for a hypothetical order of 'size' 
        ...
        Args:
            ...
            is_base_currency (bool, optional): 
                If True, 'size' is in base currency, converted to quote currency 
        ...
        while cum_size < size and level < self._size:
            ...
            available_size = min(level_size, size - cum_size)
            weighted_slippage += abs(level_price - mid_price) * available_size

            cum_size += available_size
            level += 1

        if cum_size == 0.0:
            raise RuntimeError("Total cumulative size is zero; cannot compute slippage.")

        cdef double slippage = weighted_slippage / cum_size

        # Cap slippage at mid_price (arbitrary design choice). 
        # May help prevent errors for features using this. 
        if slippage > mid_price:
            slippage = mid_price
    ...
    cpdef cnp.ndarray get_bids(self):
        ...
        Returns:
            cnp.ndarray: The 2D NumPy array representing the bid side 
                of the orderbook.
        """
        return self._bids
    ...
    cpdef cnp.ndarray get_asks(self):
        ...
        Returns:
            cnp.ndarray: The 2D NumPy array representing the ask side 
                of the orderbook.
        """
        return self._asks
    ...
    cpdef cnp.ndarray get_best_bid(self):
        ...
        Returns:
            cnp.ndarray: A 1D NumPy array [price, size] at the best bid level.
        """
        return self._bids[0]
    ...
    cpdef cnp.ndarray get_best_ask(self):
        ...
        Returns:
            cnp.ndarray: A 1D NumPy array [price, size] at the best ask level.
        """
        return self._asks[0]
    ...
    cpdef cnp.ndarray get_bbo(self):
        ...
        Returns:
            cnp.ndarray: A 2D NumPy array of shape (2, 2), where:
