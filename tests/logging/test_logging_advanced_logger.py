...
class TestLoggerConfig(unittest.TestCase):
    ...
    def test_format_string(self):
        ...
        self.assertEqual(
            formatted, "2023-01-01 12:00:00 [INFO] test_logger - Test message"
        )
...
class TestAdvancedLogger(unittest.TestCase):
    def setUp(self):
        ...
        self.file_handler = FileLogHandler(filepath=self.file_path, create=True)
    ...
    def test_data_log_method(self):
        ...
        self.assertIn(
            "{'mid_px': 100.0, 'sz': 1.0, 'px': 100.0, 'is_buy': True}", msg_content
        )
    ...
    def test_multiprocess_logging(self):
        ...
        def worker_process(worker_id):
            worker_logger = WorkerLogger(
                config=self.logger_config, name=f"worker_{worker_id}"
            )
            ...
            data = TestData(
                worker_id=worker_id, message=f"Data from worker {worker_id}"
            )
    ...
    def test_multiprocess_data(self):
        ...
        # Configure IPC transport for multiprocess communication
        self.logger_config.path = (
            f"ipc:///tmp/test_logger_multiprocess_data_{os.getpid()}"
        )
        ...
        def worker_process(worker_id):
            worker_logger = WorkerLogger(
                config=self.logger_config, name=f"data_worker_{worker_id}"
            )
        ...
        for i in range(25):
            self.assertIn(
                f"data_worker_{i}", msg_content, f"Missing data from worker {i}"
            )
            # Check for at least one data packet from each worker
            self.assertIn("Data packet", msg_content)
        ...
        for i in range(5):
            value = i * 10
            self.assertIn(
                f"value={value}", msg_content, f"Missing data with value {value}"
            )
    ...
    def test_multiprocess_high_burst_rate(self):
        ...
        def worker_process(worker_id):
            ...
            worker_logger = WorkerLogger(
                config=config, name=f"burst_worker_{worker_id}"
            )
            ...
            for i in range(BURST_COUNT):
                ...
                if level == LogLevel.TRACE:
                    worker_logger.trace(
                        f"TRACE message {i} from worker {worker_id}; seq={i}"
                    )
                elif level == LogLevel.DEBUG:
                    worker_logger.debug(
                        f"DEBUG message {i} from worker {worker_id}; seq={i}"
                    )
                elif level == LogLevel.INFO:
                    worker_logger.info(
                        f"INFO message {i} from worker {worker_id}; seq={i}"
                    )
                elif level == LogLevel.WARNING:
                    worker_logger.warning(
                        f"WARNING message {i} from worker {worker_id}; seq={i}"
                    )
                elif level == LogLevel.ERROR:
                    worker_logger.error(
                        f"ERROR message {i} from worker {worker_id}; seq={i}"
                    )
                elif level == LogLevel.CRITICAL:
                    worker_logger.critical(
                        f"CRITICAL message {i} from worker {worker_id}; seq={i}"
                    )
            ...
            for i in range(BURST_COUNT):
                data = BurstData(worker_id=worker_id, seq=i, time=time_ns())
                worker_logger.data(data)
